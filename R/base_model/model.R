...
# Resolve this script's directory so module-fx.R loads regardless of cwd.
this_dir <- tryCatch({
  a <- commandArgs(FALSE); f <- sub("^--file=", "", a[grep("^--file=", a)])
  if (length(f)) dirname(normalizePath(f)) else dirname(sys.frame(1)$ofile)
}, error = function(e) ".")
source(file.path(this_dir, "module-fx.R"))
