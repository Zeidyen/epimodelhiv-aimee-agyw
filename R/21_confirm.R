...
N <- 2000; nsteps <- 33*52; nsims <- 3; BETA <- 0.0070
...
# graded susceptibility candidates: (15-19, 20-24)
CANDS <- list(c(3.5,2.5), c(4.0,2.5), c(4.5,3.0))
best <- NULL
for (cc in CANDS) {
  p <- hetage_param(inf.prob.act=BETA, age.gap=5,
                    agyw.susc.15_19=cc[1], agyw.susc.20_24=cc[2])
  sim <- run_hetage(p, ests, N, nsteps, nsims)
  sp <- equil_prev(sim, 52); d <- distance(sp)
  cat(sprintf("susc 15-19=%.1f 20-24=%.1f  RMSE=%.3f | f15-19=%.3f f20-24=%.3f m30-34=%.3f m35-39=%.3f m40-44=%.3f\n",
      cc[1], cc[2], d, sp$f_15_19%||%NA, sp$f_20_24%||%NA, sp$m_30_34%||%NA, sp$m_35_39%||%NA, sp$m_40_44%||%NA))
  if (is.null(best) || d < best$rmse) best <- list(susc=cc, prev=sp, rmse=d, beta=BETA)
}
