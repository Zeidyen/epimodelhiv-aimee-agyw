...
para("Adolescent girls and young women (AGYW, aged 15–24 years) in sub-Saharan Africa "
     ...
     "carries one of the highest AGYW HIV burdens in the world [19]. The drivers are "
     ...
     "partnership network, and structural barriers to prevention and care [19, 20, 21]. "
     "Despite a large national ART programme and the availability of oral pre-exposure "
...
para("Pre-exposure prophylaxis is highly effective when taken [15, 16], but among AGYW "
     "its real-world impact has been limited by low uptake and poor persistence: many "
     "young women who initiate oral PrEP discontinue within months [18]. Interventions "
     "that raise "
...
para("We developed a stochastic, individual-based dynamic network model of heterosexual "
     ...
     "model was implemented in the EpiModel framework (v2.6.1) [2, 3] in R [4], building "
...
para("The model simulated a closed sexually-active population aged 15–50 years. "
     ...
     "HIV epidemics [5, 6]), and two structural constraints: (1) heterosexual mixing, imposed "
     ...
     "(“blesser”) partnership data [20, 21]. Mean partnership degree was informed by "
     "the Clover/Aimee cohort (mean 1.71 sexual partners in the past year) [1], with main "
...
para("HIV progression was modelled as susceptible → acute → chronic → "
     ...
     "[9, 10]; individuals on ART but not yet suppressed had relative infectiousness "
     "0.30, and the virally suppressed 0.01 (near-non-infectious; "
     "treatment-as-prevention [11, 12, 13, 14]). Per-partnership transmission each "
     "weekly time step was computed from a per-act transmission probability (calibrated "
     "to 0.0035; within the range from per-act transmission studies [7, 8]), the number "
     "of sex acts (5/week main, 2/week casual), the infector's stage and ART status, "
     "and — on the susceptible side — PrEP status and an age-graded elevated per-contact "
     "susceptibility for young women (2.0× at ages 15–19 and 1.5× at 20–24 [19]).")
...
para("PrEP was modelled on the susceptible side as a 95% per-act reduction in "
     "acquisition probability [15, 16, 17]. Individuals became PrEP-indicated if they "
     "had a partnership degree ≥ 2, an HIV-positive partner, or (for reached AGYW from "
     "2025) were designated priority-population eligible. Indicated susceptibles "
     "initiated PrEP at 0.005 per week and discontinued at 0.027 per week — a median "
     "time on PrEP of ~6 months, set to reflect the documented poor continuation of "
     "oral PrEP among "
     "South African AGYW [18], so that baseline PrEP coverage remained low (~3%) with rapid "
...
para("The model was calibrated using the trajectory-fitting approach standard for South "
     ...
     "reproduced the Thembisa v5.0 targets [24] across 1990–2022. The best-fitting value "
